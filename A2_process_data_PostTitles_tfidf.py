...
# Clean 'Text' column
df['Cleaned Post Title'] = df['Post Title'].apply(clean_text)
...
# Create a TfidfVectorizer object to generate word embeddings
vectorizer = TfidfVectorizer()

post_titles = df['Cleaned Post Title'].unique()

# Generate the document-term matrix
X = vectorizer.fit_transform(post_titles)
...
#
# plt.figure()
# plt.plot(range(2, 25), ch_scores, 'bx-')
...
#
# plt.figure()
# plt.plot(range(2, 25), db_scores, 'bx-')
...
new_df = pd.DataFrame({'Cleaned Post Title': post_titles, 'Cluster ID': kmeans.labels_})

# Merge the new DataFrame with the original DataFrame on the 'Post Title' column
merged_df = pd.merge(df, new_df, on='Cleaned Post Title')

# Extract the top 10 topics for each cluster
cluster_topics = []
for i in range(4):
    cluster_df = merged_df[merged_df['Cluster ID'] == i]
    cluster_X = vectorizer.transform(cluster_df['Cleaned Post Title'])
...
merged_df.to_csv('C:/Users/amand/OneDrive/Desktop/Thesis/Updated_Thesis/postTitle_tfidf_kmeans_optimization.csv', index=False)