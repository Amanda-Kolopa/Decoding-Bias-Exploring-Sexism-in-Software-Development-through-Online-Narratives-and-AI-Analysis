...
# Apply LSA to extract topics from the text data
lsa = TruncatedSVD(n_components=25, algorithm='randomized', n_iter=100, random_state=42)
lsa.fit(X)
...
for i in range(25):
    ...
    cluster_lsa = lsa.transform(cluster_X)
    order_cluster_topics = cluster_lsa.argsort()[:, ::-1]
    top_cluster_topics = []
    for j in range(5):
...
merged_df.to_csv('C:/Users/amand/OneDrive/Desktop/Thesis/Updated_Thesis/Model Scripts/Outputs/LSA - TI.csv', index=False)