...
vectorizer = TfidfVectorizer(stop_words='english', vocabulary=categories['testimonial injustice'][35:])
...
plt.figure()
plt.plot(range(2, MAX_TOPICS), sil, 'bx-')
...
plt.figure()
plt.plot(range(2, MAX_TOPICS), ch_scores, 'bx-')
...
plt.figure()
plt.plot(range(2, MAX_TOPICS), db_scores, 'bx-')
...
# kmeans = KMeans(n_clusters=25, max_iter=1000).fit(X)
#
...
# merged_df.to_csv('C:/Users/amand/OneDrive/Desktop/Thesis/Updated_Thesis/Model Scripts/Outputs/LDA - TI.csv', index=False)